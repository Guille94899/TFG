...
def per_label_report(labels, preds, class_names):
    report = []
    for i, name in enumerate(class_names):
        ...
        report.append((name, float(p), float(r), float(f1), support))
    return report
...
split_1 = tokenized.train_test_split(test_size=0.10, seed=42)
train_val_ds = split_1["train"]
test_ds = split_1["test"]
val_ratio = 0.10 / 0.90  
split_2 = train_val_ds.train_test_split(test_size=val_ratio, seed=42)
train_ds = split_2["train"].with_format("torch")
val_ds = split_2["test"].with_format("torch")
...
print("\n📌 Evaluación final en TEST (modelo definitivo):")
...
report = per_label_report(labels, preds, list(mlb.classes_))

print("\n📊 Métricas por etiqueta (TEST):")
print("Etiqueta\t\tPrecision\tRecall\t\tF1\t\tSupport")
for name, p, r, f1, sup in report:
    print(f"{name:16s}\t{p:.3f}\t\t{r:.3f}\t\t{f1:.3f}\t\t{sup}")
...
print(f"\n✅ Modelo RoBERTa EP FINAL  guardado en: {SAVE_DIR}")
